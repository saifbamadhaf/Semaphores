...
void checkoutScanner(int customer_id, int chosen_cashier);
...
void *customerScanner(void *arg) {
    ...
    if (scannersAvailable > 0) {
        ...
        int chosen_terminal = 0;

        // Choose the terminal with the no queue
        for (int i = 1; i < NUM_TERMINAL; i++) {
            sem_wait(&terminal_Semaphores[i]); // Wait on semaphore for each cashier

            // Check if current cashier's queue is smaller or non-existent
            if (cashier_queue[i] < cashier_queue[chosen_terminal]) {
                chosen_terminal = i;
            }
            sem_post(&terminal_Semaphores[i]); // Release semaphore for each cashier
        }
        int queue = cashier_queue[0] + cashier_queue[1];

        sem_wait(&terminal_queue_mutex[chosen_terminal]);

        if(cashier_queue[chosen_terminal] == 0){
            cashier_queue[chosen_terminal]++;
